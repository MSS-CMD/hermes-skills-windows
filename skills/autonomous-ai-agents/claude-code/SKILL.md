...
This skill covers **Claude Code**. Hermes also supports two other autonomous coding agent CLIs. Reference content from the absorbed skills is preserved in `references/`:

### Codex (OpenAI)

For tasks where Claude Code isn't available or the user prefers OpenAI. See `references/codex.md` for the full reference.
...
For tasks where you want a provider-agnostic, open-source coding agent. See `references/opencode.md` for the full reference.
...
| Agent | Key Strength | Auth Needed |
|-------|-------------|-------------|
| **Claude Code** (this skill) | Rich orchestration, MCP, thinking | Anthropic API key or OAuth |
| **Codex** (OpenAI) | Simple one-shot, --yolo mode | OpenAI API key or OAuth |
| **OpenCode** (open-source) | Provider-agnostic, multi-model | Any LLM provider key |
