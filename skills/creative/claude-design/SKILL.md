...
## Sketching — Throwaway HTML Mockups (formerly `sketch` skill)

For **early exploration** when the user wants to "see what it could look like" before committing to a design direction. The point is 2-3 quick interactive HTML variants so the user can compare visual directions side-by-side.

**When to sketch vs when to design:** Sketch when user says "sketch this", "show me 2-3 takes", "compare layouts A vs B". Design (the main claude-design skill) when the brief is settled and you're producing the final artifact.

### Method

1. Generate 2-3 HTML variants in a temp directory as `sketch-variant-1.html`, `sketch-variant-2.html`, etc.
2. Each variant should explore a different layout direction or visual approach
3. Keep production-quality code: clean spacing, real color, responsive viewport. "Throwaway" means no polishing round, not sloppy code
4. Note which variant you recommend and why
5. Clean up the temp files when done

### Sketching vs Other Creative Skills

| Skill | When |
|-------|------|
| **sketch** (this section) | Exploration, 2-3 variants for comparison, disposable |
| **claude-design** (main) | Production-quality one-off artifact (landing page, deck, prototype) |
| **popular-web-designs** | "Make it look like Stripe/Linear/Vercel" — paste a known brand's design system |
| **excalidraw / architecture-diagram** | Diagrams, not UI mockups |
